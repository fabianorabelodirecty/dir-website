...
const AboutSection: React.FC<{ assets: About[] }> = ({ assets }) => {
    ...
    return (
        <div ref={sectionRef} className="relative w-full h-[100svh] flex items-center justify-center bg-black">
            <motion.div style={{ width, x, y }} className={`absolute top-0 left-0 hidden xl:flex`}>
                <img
                    ref={imgRef}
                    src={imgAboutWork}
            ...
            <section
                id="about"
                className="w-full max-w-[1280px]   px-4 flex flex-col xl:flex-row relative z-10 mb-[12svh] -mt-96 xl:mt-auto "
            >
                <Fade className="xl:w-[45%] flex mb-0 mt-auto">
